...
SELECT id AS idpost , title AS titlepost ,deatecreate FROM post  ORDER BY  datecreate DESC ; 
...
SELECT id AS idpost , title AS titlepost , LEFT(content ,255) AS contentshort , datecreate FROM post WHERE user_id IS NOT NULL ORDER BY  datecreate DESC ; 
...
SELECT id AS idpost , title AS titlepost , LEFT(content ,255) AS contentshort , datecreate FROM post WHERE user_id IS NOT NULL AND visible =1 ORDER BY  datecreate DESC ; 
...
SELECT id, username,userscreen FROM user ORDER BY userscree ASC ; 
...
# Il faut utiliser le nom des tables pour éviter qu'il y ai une collision d'id !
 ...
 select post.id , post.title ,post.datecreate,user.userscreen  FROM post INNER JOIN user ON user.id = post.user_id ORDER BY post.datecreate ASC ;  
# idem avec alias internes (alias de tables)
 select p.id , p.title ,p.datecreate,u.userscreen  FROM post AS p INNER JOIN user as u ON u.id = p.user_id ORDER BY p.datecreate ASC ;  
...
SELECT p.id , p.title , p.datecreate, u.userscreen  FROM post AS p LEFT JOIN user AS u ON  u.id = p.user_id ORDER BY p.datecreate ASC ; 
...
SELECT p.id , p.title , p.datecreate, u.userscreen  FROM post AS p RIGHT JOIN user AS u ON  u.id = p.user_id ORDER BY p.datecreate ASC ; 
